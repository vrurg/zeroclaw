...
| `ask_user` | Send a question to the active channel and wait for a reply. Supports optional `choices` for structured responses (inline keyboard on Telegram, numbered list on CLI). On ACP, `choices` are required: free-form ask awaits the ACP elicitation RFD. During a parent Goal turn, it instead records a typed Goal interruption; the controller presents the completed turn and pauses, and the user replies with `/goal resume RESPONSE`. Goal-owned children cannot call it because only the parent may interrupt the session. Parameters: `question` (required), `choices` (optional list), `timeout_secs` (default 300). |
