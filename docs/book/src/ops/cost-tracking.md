...
trusted goal context, the controller binds the exact admitted durable task
before provider I/O and validates that task against canonical storage. Usage
rows are stamped with the bound task id; verifier calls and synchronous child
work inherit that exact-task binding. For JSONL compatibility the serialized
key is `goal_task_id`, but the in-memory API is task-generic so future task
features can reuse the same accounting path.
