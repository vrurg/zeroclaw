...
| Durable task control plane | Canonical task records plus kind-specific extension rows | `data/control_plane.db` | Process-global `ControlPlaneHandle` plus the supervision reaper | A process restart rotates the boot id and applies task recovery policy; an in-process reload keeps the boot id and requeues still-running goals only after their old channel workers confirm shutdown. Recovered turns remain bound to their original task id and recheck its visible `running` state before starting the model turn. Recovery ownership remains live through admission, permit waits, model execution, and chained continuations, and restores the task id if cancellation leaves it running. If the next iteration cannot build an executor owned by the goal's agent, queued goals are paused with a restart-recovery blocker. A missed shutdown deadline, recovery-query failure, or failed durable recovery transition rejects the unsafe handoff and retains unprocessed recovery work | Lifecycle, ownership, route, principal, and timestamps belong to task records. Goal objectives, limits, pause reasons, and blockers belong to the goal extension. Recovery queues are transient delivery state, not lifecycle authority. |
...
- Durable task lifecycle and recovery: `crates/zeroclaw-runtime/src/control_plane/`
