...
3. The concrete provider parser translates its protocol's completion semantics into `Final` or an error. Most guarded SSE parsers require their configured completion signal. Anthropic requires `message_stop`; EOF after a `message_delta.stop_reason` is incomplete and returns an error.
