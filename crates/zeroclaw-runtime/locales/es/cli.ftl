...
cli-quickstart-error-anthropic-setup-token-required = La autenticación con setup-token de Anthropic requiere un token de configuración
cli-quickstart-error-anthropic-setup-token-store = no se pudo guardar el token de configuración de Anthropic: {$err}
...
cli-agent-not-created-disk-state-uncertain = Tu agente no fue creado. La configuración no se guardó, pero se produjo un error al revertir la credencial de Anthropic; inspecciona el perfil almacenado antes de volver a intentarlo.
...
cli-config-patch-warning-legacy-colon-alias = advertencia: {$path} es un alias heredado de proveedor que sigue sin ser válido y debe repararse por separado.
cli-config-patch-warning-pre-existing-validation = advertencia: {$path} conserva un error de validación no relacionado y debe repararse por separado: {$warning}
