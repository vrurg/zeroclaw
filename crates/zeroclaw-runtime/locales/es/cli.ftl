...
cli-approval-prompt-yesno = { "   " }[Y] Sí / [N] No:{ " " }
...
channel-runtime-session-prompt-load-failed = ⚠️ No pude cargar el contexto persistente de la sesión, por lo que esta solicitud no se envió al modelo.
...
channel-approval-reply-instruction-yesno-once = Responde: "{ $yes_command }" o "{ $no_command }"
channel-approval-reply-instruction-approve-deny-once = Responde con `{ $approve_command }` / `{ $deny_command }`.
...
session-prompt-approval-heading = Aprobar modificación de la instrucción persistente de la sesión
