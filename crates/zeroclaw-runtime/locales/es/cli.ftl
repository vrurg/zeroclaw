...
cli-quickstart-auth-anthropic-skip-hint = {"  "}Termina más tarde con: `claude setup-token`, luego `zeroclaw auth setup-token --model-provider anthropic --profile {$alias}` y configura `providers.models.anthropic.{$alias}.auth_mode = "oauth"`
...
cli-quickstart-error-anthropic-setup-token-store = no se pudo guardar el token de configuración de Anthropic
