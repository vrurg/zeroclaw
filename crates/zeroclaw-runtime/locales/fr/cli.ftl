...
cli-quickstart-anthropic-auth-mode-help = Choisissez `api_key` pour une clé Anthropic Console, ou `setup_token` pour enregistrer un jeton de `claude setup-token` dans le profil d'authentification lié à l'alias, avec `auth_mode = "oauth"`.
cli-quickstart-anthropic-api-key-help = Collez une clé d'API Anthropic Console. Pour utiliser Claude Max, exécutez d'abord `claude setup-token`, puis choisissez `setup_token`.
...
cli-quickstart-auth-anthropic-skip-hint = {"  "}Quickstart n'a pas été écrit. Relancez Quickstart et choisissez `setup_token` pour terminer la configuration d'Anthropic pour {$alias}.
...
cli-quickstart-error-anthropic-setup-token-required = L'authentification setup-token Anthropic requiert un jeton de configuration
cli-quickstart-error-anthropic-setup-token-store = échec de l'enregistrement du jeton de configuration Anthropic : {$err}
...
cli-quickstart-warning-personality-write-failed = Quickstart est terminé, mais le fichier de personnalité {$path} n'a pas pu être écrit : {$err}
...
cli-config-patch-warning-legacy-colon-alias = avertissement : {$path} est un alias de fournisseur hérité qui reste non valide et doit être réparé séparément.
cli-config-patch-warning-pre-existing-validation = avertissement : {$path} conserve une erreur de validation sans lien et doit être réparé séparément : {$warning}
