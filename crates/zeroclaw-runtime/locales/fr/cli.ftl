...
cli-quickstart-anthropic-api-key-help = Collez une clé d'API Anthropic Console. Avec `setup_token`, collez ici le jeton généré par `claude setup-token` ; Quickstart l'enregistre dans le profil d'authentification lié à l'alias.
...
cli-quickstart-auth-anthropic-skip-hint = {"  "}Quickstart n'a pas été écrit. Pour terminer plus tard, relancez Quickstart afin de créer {$alias}, puis exécutez `claude setup-token`, `zeroclaw auth setup-token --model-provider anthropic --profile {$alias}` et définissez `providers.models.anthropic.{$alias}.auth_mode = "oauth"`.
...
cli-quickstart-error-reload-pending = Quickstart attend que le démon se recharge après une soumission précédente réussie
...
cli-quickstart-error-anthropic-setup-token-store = échec de l'enregistrement du jeton de configuration Anthropic
