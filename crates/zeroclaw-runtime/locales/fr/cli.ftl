...
channel-runtime-session-prompt-budget-exceeded = ⚠️ Le contexte persistant de la session dépasse le budget du prompt système ; cette requête n’a donc pas été envoyée au modèle.
channel-runtime-session-prompt-load-failed = ⚠️ Je n’ai pas pu charger le contexte persistant de la session ; cette requête n’a donc pas été envoyée au modèle.
...
channel-approval-reply-instruction-yesno-once = Répondez : "{ $yes_command }" ou "{ $no_command }"
channel-approval-reply-instruction-approve-deny-once = Répondez par `{ $approve_command }` / `{ $deny_command }`.
...
session-prompt-approval-heading = Approuver la modification de l’invite persistante de session
