...
cli-quickstart-auth-anthropic-skip-hint = {"  "}クイックスタートは書き込まれませんでした。クイックスタートを再実行し、{$alias} の Anthropic 設定を完了するには `setup_token` を選択してください。
...
cli-config-patch-warning-legacy-colon-alias = 警告: {$path} は無効なままのレガシープロバイダーエイリアスであり、個別に修復する必要があります。
cli-config-patch-warning-pre-existing-validation = 警告: {$path} には無関係な検証エラーが残っており、個別に修復する必要があります: {$warning}
