...
channel-runtime-session-prompt-load-failed = ⚠️ 永続セッションコンテキストを読み込めなかったため、このリクエストはモデルに送信されませんでした。
...
channel-approval-reply-instruction-yesno-once = 「{ $yes_command }」または「{ $no_command }」と返信してください。
channel-approval-reply-instruction-approve-deny-once = 「{ $approve_command }」/「{ $deny_command }」と返信してください。
...
session-prompt-approval-heading = 永続セッションプロンプトの変更を承認
