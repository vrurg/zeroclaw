...
goal-mode-paused-blocked = ⏸️ Goal paused: the verifier requires resolution. Use `/goal status` to review the details. If it requests information, send it as a normal message in this session. If it names an external dependency, resolve that first. Otherwise resolve the blocker. Then use `/goal resume` to continue, or `/goal cancel` to dismiss this Goal.
...
goal-mode-yes = yes
goal-mode-no = no
goal-mode-details = Status: { $status }; epoch: { $epoch }; accounting: { $accounting }; resumable: { $resumable }; token limit: { $token_limit }; cost limit USD: { $cost_limit }; pause reason: { $pause_reason }.
