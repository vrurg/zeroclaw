...
goal-command-tool-approval-required-blocker = Tool `{$tool}` requires operator approval before this goal can continue.
goal-command-tool-approval-required-status = ⏸️ Goal paused because tool `{$tool}` requires operator approval.
...
goal-command-restart-recovery-execution-gap-blocker = Restart recovery stopped before the goal controller settled lifecycle state.
...
goal-command-provider-initialization-paused-description = The selected provider could not be initialized while resuming this goal.
goal-command-provider-initialization-blocker = The selected provider could not be initialized. Update its configuration and resume the goal.
