...
cli-quickstart-auth-anthropic-skip-hint = {"  "}Quickstart was not written. Rerun Quickstart and choose `setup_token` to finish Anthropic setup for {$alias}.
...
cli-agent-not-created-disk-state-uncertain = Your agent was not created. The config was not saved, but the Anthropic credential rollback failed; inspect the stored profile before retrying.
...
cli-quickstart-warning-personality-write-failed = Quickstart completed, but could not write personality file {$path}: {$err}
...
cli-config-patch-warning-legacy-colon-alias = warning: {$path} is a legacy provider alias that remains invalid and must be repaired separately.
cli-config-patch-warning-pre-existing-validation = warning: {$path} retains an unrelated validation error and must be repaired separately: {$warning}
