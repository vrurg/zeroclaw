...
mod tests {
    ...
    fn goal_resume_test_config(agent: &str) -> zeroclaw_config::schema::Config {
    ...
    fn goal_resume_test_context(
        agent: String,
        route: String,
        principal: String,
    ) -> GoalAdmissionContext {
        GoalAdmissionContext::new(agent)
    ...
    async fn tool_resumes_goal_with_untrusted_reason() {
        ...
        control_plane
            ...
            .await
            .unwrap();
        let config = goal_resume_test_config(&agent);
        let tool = GoalResumeTool::new(agent.clone(), std::sync::Arc::new(config));
        let owner = goal_resume_test_context(agent, route, principal);
