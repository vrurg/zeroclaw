...
mod tests {
    ...
    async fn peer_turn_cost_scope_records_recipient_usage_and_enforces_shared_budget() {
        ...
        let unscoped = record_tool_loop_cost_usage("mock-provider", "test-model", &usage)
            .await
            .expect("unscoped accounting must not fail");
        assert!(
            unscoped.is_none(),
        ...
        let recorded = deliver_peer_turn_with_cost_scope(
            ...
            async { record_tool_loop_cost_usage("mock-provider", "test-model", &usage).await },
        )
        .await;

        let (total_tokens, cost_usd) = recorded
            .expect("scoped accounting must not fail")
            .expect("scoped consumer must record the recipient's usage");
        ...
        assert!(
            sender_usage
                .expect("sender accounting must not fail")
                .is_some(),
