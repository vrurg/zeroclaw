...
/// Future returned by the live approval-denial policy resolver.
pub type GoalApprovalDenyBehaviorFuture =
    Pin<Box<dyn Future<Output = Result<GoalApprovalDenyBehavior>> + Send>>;
/// Async resolver that reconciles and reads approval-denial policy at decision time.
pub type GoalApprovalDenyBehaviorResolver =
    Arc<dyn Fn() -> GoalApprovalDenyBehaviorFuture + Send + Sync>;
...
pub struct GoalRuntimeScope {
    ...
    /// Live resolver for the global denial policy at decision time.
    approval_deny_behavior: Option<GoalApprovalDenyBehaviorResolver>,
...
impl GoalRuntimeScope {
    ...
    pub fn with_approval_deny_behavior_resolver(
        mut self,
        resolver: GoalApprovalDenyBehaviorResolver,
    ) -> Self {
...
pub async fn current_goal_approval_deny_behavior() -> Result<GoalApprovalDenyBehavior> {
    let resolver = GOAL_RUNTIME_SCOPE
        .try_with(|scope| scope.approval_deny_behavior.clone())
        .ok()
        .flatten();
    match resolver {
        Some(resolver) => resolver().await,
        None => Ok(GoalApprovalDenyBehavior::default()),
    }
}
