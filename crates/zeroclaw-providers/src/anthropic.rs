...
impl AnthropicModelProvider {
    ...
    fn apply_auth(
        ...
        match anthropic_beta_features(authorization, thinking_display_beta, extra_betas) {
...
impl AnthropicModelProvider {
    ...
    fn stream_prepared(
        ...
        let thinking_display_beta = prepared
            .thinking
            .as_ref()
            .is_some_and(|config| config.display.is_some());

        if prepared.thinking.is_some() && !thinking_display_beta {
            ::zeroclaw_log::record!(
                INFO,
                ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Note)
                    .with_attrs(::serde_json::json!({
            ...
            .flat_map(|result| match result {
                Ok(resp) => {
                    ...
                    if let Some(text) = resp.text.filter(|text| !text.is_empty()) {
            ...
            ::zeroclaw_log::record!(
                ...
                ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Note)
                    .with_attrs(::serde_json::json!({
                        ...
                        "tools_count": tools_count,
                        "tool_choice": prepared.tool_choice.as_ref().and_then(|value| value.get("type")).and_then(|value| value.as_str()),
                        "thinking_enabled": prepared.thinking.is_some(),
    ...
    fn stream_chat_impl(
        ...
        let use_native_thinking_fallback = prepared
            .thinking
            .as_ref()
            .is_some_and(|thinking| thinking.display.is_none());
        let transport = match self.stream_transport(use_native_thinking_fallback) {
