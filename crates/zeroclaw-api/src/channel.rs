...
pub trait Channel: Send + Sync + crate::attribution::Attributable {
    ...
    ///
    /// This is one canonical address for shared, best-effort helpers
    /// such as prompt identity and transitional addressed-command
    /// stripping. It is not a complete parser for every address form a
    /// platform accepts. Channel-specific aliases, rich mention syntax,
    /// and client quirks must stay in the channel implementation until
    /// command ingress grows an explicit channel-bound abstraction.
